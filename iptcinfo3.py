...
def jpeg_get_variable_length(fh):
    ...
    if length < 2:
        logger.error("jpeg_get_variable_length: erroneous JPEG marker length")
        return 0
    return length - 2
...
def jpeg_next_marker(fh):
    ...
    try:
        ...
        while ord3(byte) != 0xff:
            # logger.error("jpeg_next_marker: bogus stuff in Jpeg file at: ')
            byte = read_exactly(fh, 1)
    ...
    with open(filename, 'wb') as fh:
        ...
        if not (ord3(ff) == 0xff and ord3(soi) == SOI):
            ...
            while True:
                ...
                if ord3(marker) == 0:
                    logger.error("Marker scan failed")
                    break
                ...
                if not jpeg_skip_variable(fh):
                    logger.error("jpeg_skip_variable failed")
                    return None
...
class IPTCInfo:
    ...
    def __init__(self, fobj, force=False, inp_charset=None, out_charset=None):
        ...
        with smart_open(self._fobj, 'rb') as fh:
            ...
            if datafound or force:
                ...
                logger.error('No IPTC data found in %s', fobj)
    ...
    def scanToFirstIMMTag(self, fh):
        ...
        if file_is_jpeg(fh):
            ...
            logger.error("File not a JPEG, trying blindScan")
    ...
    def blindScan(self, fh, MAX=8192):
        ...
        while offset <= MAX:
            try:
                ...
                logger.error("BlindScan: hit EOF while scanning")
                return None
                ...
                if record == 1 and dataset == 90:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.error('WARNING: problems with charset recognition (%r)', temp)
                            cs = None
                        if cs in c_charset:
        ...
        while True:
            ...
            if self.inp_charset:
                try:
                    ...
                    logger.error('Data "%r" is not in encoding %s!', value, self.inp_charset)
    ...
    def packedIIMData(self):
        ...
        for dataset, value in self._data.items():
            ...
            if not (isinstance(dataset, int) and dataset in c_datasets):
                logger.error("packedIIMData: illegal dataname '%s' (%d)", dataset, dataset)
                continue
